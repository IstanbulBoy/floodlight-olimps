...
public class DeviceManagerImpl implements
    ...
    protected IOlimpsTopologyService topology;
    ...
    protected static class DeviceUpdate {
        ...
        public DeviceUpdate(Device device, Change change, EnumSet<DeviceField> fieldsChanged) {
            super();
            this.device = device;
            this.change = change;
        ...
        public String toString() {
            String devIdStr = device.getEntityClass().getName() + "::" + device.getMACAddressString();
    ...
    public void init(FloodlightModuleContext fmc) throws FloodlightModuleException {
        ...
        this.topology = fmc.getServiceImpl(IOlimpsTopologyService.class);
    ...
    public void startUp(FloodlightModuleContext fmc) throws FloodlightModuleException {
        ...
        Runnable consolidateStoreRunner = new Runnable() {
            ...
            public void run() {
                ...
                storeConsolidateTask.reschedule(syncStoreConsolidateIntervalMs, TimeUnit.MILLISECONDS);
            }
        };
    ...
    protected Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx) {
       ...
       if (logger.isTraceEnabled()) {
           logger.trace("Received PI: {} on switch {}, port {} *** eth={} *** srcDev={} *** dstDev={} *** ",
                        new Object[] { pi, sw.getStringId(), pi.getInPort(), eth, srcDevice, dstDevice });
       }
    ...
    private int getSrcNwAddr(Ethernet eth, long dlAddr) {
        if (eth.getPayload() instanceof ARP) {
            ...
            if ((arp.getProtocolType() == ARP.PROTO_TYPE_IP) && (Ethernet.toLong(arp.getSenderHardwareAddress()) == dlAddr)) {
     ...
     */
    protected Entity getSourceEntityFromPacket(Ethernet eth, long swdpid, short port) {
        ...
        int virtualInPortId = OFSwitchPort.virtualPortIdOf(port, vlan);
     ...
     * address in ARP data.
     */
    protected void learnDeviceFromArpResponseData(Ethernet eth, long swdpid, short port) {
    ...
    protected Device learnDeviceByEntity(Entity entity) {
        ...
        while (true) {
            ...
            if (deviceKey != null) {
                ...
                if (device == null) {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug("No device for deviceKey {} while while processing entity {}", deviceKey, entity);
                    }
                ...
                if (entity.hasSwitchPort() && !topology.isAttachmentPointPort(entity.getSwitchDPID(), entity.getSwitchPort())) {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug("Not learning new device on internal link: {}", entity);
                    }
                    device = null;
                    break;
                }
                ...
                if (!isEntityAllowed(entity, entityClass)) {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug("PacketIn is not allowed {} {}", entityClass.getName(), entity);
                    }
                    device = null;
                    break;
                }
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("New device created: {} deviceKey={}, entity={}", new Object[]{device, deviceKey, entity});
                }
            ...
            if (!isEntityAllowed(entity, device.getEntityClass())) {
                cntPacketNotAllowed.updateCounterNoFlush();
                if (logger.isDebugEnabled()) {
                    logger.info("PacketIn is not allowed {} {}", device.getEntityClass().getName(), entity);
                }
                return null;
            }
            ...
            // the chain.
            //if (entity.hasSwitchPort() && !topology.isAttachmentPointPort(entity.getSwitchDPID(), entity.getSwitchPort().shortValue())) {
            if (entity.hasSwitchPort() && !topology.isAttachmentPointPort(entity.getSwitchDPID(), entity.getSwitchPort())) {
    ...
    public void topologyChanged(List<LDUpdate> updateList) {
        ...
        if (updateList != null) {
        	for (LDUpdate ldu : updateList) {
        		switch (ldu.getOperation()) {
        			case SWITCH_REMOVED:
        				diter = deviceMap.values().iterator();
        				while (diter.hasNext()) {
        		            Device d = diter.next();
        		            if (d.updateAttachmentPoint()) {
        		                if (logger.isDebugEnabled()) {
        		                    logger.debug("Attachment point removed for device: {}", d);
        		                }
        		                this.deleteDevice(d);
        		                sendDeviceRemovedNotification(d);
        		            }
        		        }
        				break;
        			default:
     ...
     * @param updates the updates to process.
     */
    protected void sendDeviceRemovedNotification(Device d) {
        cntDeviceMoved.updateCounterNoFlush();
        deviceSyncManager.storeDevice(d);
        List<IDeviceListener> listeners = deviceListeners.getOrderedListeners();
        if (listeners != null) {
