...
public class SwitchSyncRepresentation {
    public static class SyncedPort {
        ...
        public static SyncedPort fromOFSwitchPort(OFSwitchPort p) {
        	SyncedPort rv = new SyncedPort();
            rv.portNumber = p.getOFPhysicalPort().getPortNumber();
            rv.hardwareAddress = p.getHardwareAddress();
            rv.name = p.getOFPhysicalPort().getName();
            rv.config = EnumBitmaps.toBitmap(p.getConfig());
            rv.state = p.getOFPhysicalPort().getState();
            if (p.isLinkDown())
                rv.state |= OFPortState.OFPPS_LINK_DOWN.getValue();
            rv.currentFeatures  = EnumBitmaps.toBitmap(p.getCurrentFeatures());
            rv.advertisedFeatures =
    ...
    public SwitchSyncRepresentation(OFFeaturesReply fr,
        ...
        this.ports = toSyncedPortList(OFSwitchPort.ofSwitchPortListOf(fr.getPorts()));
    ...
    private static List<SyncedPort> toSyncedPortList(Collection<OFSwitchPort> ports) {
        List<SyncedPort> rv = new ArrayList<SyncedPort>(ports.size());
        for (OFSwitchPort p: ports) {
            rv.add(SyncedPort.fromOFSwitchPort(p));
        }
        return rv;
    }
